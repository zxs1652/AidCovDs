...
function dis_Matrix = compute_Dis_Par(option,worker_th,workers)
    ...
    dis_Matrix_Output = [option.mat_Path,'\','disMatrix','_','par',num2str(worker_th),'_',option.name_Dataset,'.mat'];
    if ~exist(dis_Matrix_Output,'file') && begin_Class <= end_Class
        for cla_rth = begin_Class:end_Class           
            for set_rth = 1:option.num_Sample
                row_th = row_th + 1;
                r_spd_CovDs = Cov_RandomD{cla_rth,set_rth};
                r_log_CovDs = Log_RandomD{cla_rth,set_rth};
                r_inv_CovDs = Inv_RandomD{cla_rth,set_rth};
            %%  for columns
                for cla_cth = begin_Class:num_Classes
                    for set_cth = 1:option.num_Sample
                        col_th = col_th + 1;
                        if col_th >= row_th
                            c_spd_CovDs = Cov_RandomD{cla_cth,set_cth};
                            c_log_CovDs = Log_RandomD{cla_cth,set_cth};
                            c_inv_CovDs = Inv_RandomD{cla_cth,set_cth};
