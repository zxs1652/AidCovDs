function descrs = LassoSPMencoderImage(encoder, im, varargin)
...
all_idx = 1: size(covariances,2);
singular_idx = [];
for i = 1:size(covariances,2)
    if numel(find(covariances(:,i) < 6e-6)) > 30
        singular_idx = [singular_idx,i];
    end
end
idx = setdiff(all_idx,singular_idx);
means = means(:,idx);
covariances = covariances(:,idx);
means = bsxfun(@times, means, 1./sqrt(sum(means.^2))) ;
covariances = bsxfun(@times, covariances, 1./sqrt(sum(covariances.^2))) ;
...
codebook = bsxfun(@times, codebook, 1./sqrt(sum(codebook.^2))) ;
...
X = [];
arr=[];x=[];y=[];
for c = 1:length(binsize)
    numthreshold = (binsize(c)/2)^2*0.8;
    step = 8;
    minx = min(features.frame(1,:));
    maxx = minx + binsize(c);
    miny = min(features.frame(2,:));
    maxy = miny + binsize(c);
    while(maxy<imageSize(1))
        ...
        %       [means, covariances] = vl_gmm(descrs, 1, 'verbose', 'Initialization', 'kmeans', 'CovarianceBound', double(max(v)*0.0001), 'NumRepetitions', 1) ;
              ...
              idx = find(covariances < eps(-4));
              cnt = cnt + numel(idx);
              covariances(idx) = eps(-4);
              means = bsxfun(@times, means, 1./max(1e-12, norm(means)));
              covariances = bsxfun(@times, covariances, 1./max(1e-12, norm(covariances))) ;
...
%               X = [means,covariances];   
    ...
    % fea = LLC_coding_appr(codebook',features.descr',knn,beta);
    fea = mexLasso(arr',codebook,param);
    fea = full(fea)';
    % psi = LLC_pooling(fea,features.frame(1,:),features.frame(2,:),SPM_layouts);
    psi = [psi,single(LLC_pooling(fea,x,y,SPM_layouts))];
...
psi = bsxfun(@times, psi, 1./max(1e-12, norm(psi))) ;
